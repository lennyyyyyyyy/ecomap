...
for i in np.arange(0, 2345-50, 15):
    for j in np.arange(0, 2334-50, 15):
        ...
        if applicable_squares > 1250: # 50% of the square is applicable
            pdensity_square = np.copy(pdensity[i:i+50, j:j+50])
            vegetation_square = np.copy(vegetation[i:i+50, j:j+50])
            water_square = np.copy(water[i:i+50, j:j+50])
            temps_square = np.copy(temps[i+13:i+37, j+13:j+37])
            for k in range(4):
                ...
                # combined_square = np.stack((vegetation_square, water_square, applicable_square), axis=-1)
                # combined_squareT = np.stack((vegetation_square.T, water_square.T, applicable_square.T), axis=-1)
